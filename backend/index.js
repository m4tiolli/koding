...
app.post("/responsavel", (req, res) => {
    ...
    db.query("INSERT INTO responsavel (nome, cpf, telefone, email, senha) VALUES (?, ?, ?, ?, ?)", [nome, cpf, telefone, email, senha], (err, result) => {
        if (err) {
...
app.put("/responsavel/:id", (req, res) => {
    ...
    db.query("UPDATE responsavel SET nome = ?, senha = ? WHERE id = ?", [nome, senha, userId], (err, result) => {
        if (err) {
